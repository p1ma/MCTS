...
\subsection{Abstrait}
	...
	\subsubsection{Double progressive widening} \label{dpw}
		\subsubsubsection{Principe} \label{dpw:principe}
			...
			De ce fait, en un seul appel, la fonction double progressive widening va produire à partir d'un noeud $s$ (potentiellement) 2 noeuds.

			Le premier noeud correspondra à la meilleure action possible (même principe que PW), puis le second noeud correspondra à un bruit parasite que l'on appliquera à l'action appartenant au premier noeud.

			Afin de savoir si l'on doit ajouter un bruit, le double progressive widening va donc utiliser le nombre de simulation de l'état $s'$
		...
		\subsubsubsection{Pseudo-code} \label{dpw:code}
			...
			Et voici, ci-dessous, l'algorithme MCTS avec la stratégie double progressive widening.
