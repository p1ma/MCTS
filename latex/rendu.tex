...
\subsection{Abstrait}
	...
	Voici une illustration du problème que nous allons essayer de résoudre : \\
	...
	\centering
		...
		% double fleche a
		\draw[<->] (0.75, 1) -- (0.75, 2) node[pos=.5, left] {a};

		% double fleche l
		\draw[<->] (1, 2.25) -- (2, 2.25) node[pos=.5, above] {l};

		% double fleche w
		\draw[<->] (2, 0.75) -- (2.7, 0.75) node[pos=.5, below] {w};

		% double fleche h
		\draw[<->] (5.25, 1) -- (5.25, 3) node[pos=.5, left] {h};
